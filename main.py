...
# Jinja helpers
def fmt2(v: Any) -> str:
    """Format numbers with max 2 decimal places (trim trailing zeros)."""
    if v is None:
        return ""
    try:
...
def dtfmt(v: Any) -> str:
    """Format datetime values as 'HH:MM - DD/MM/YYYY'."""
...
# Global finalize: keep ints, format floats to max 2 dp
...
def redirect(url: str) -> RedirectResponse:
    """Return a 303 redirect response (post/redirect/get friendly)."""
    return RedirectResponse(url, status_code=303)


def get_sample_readings(db: sqlite3.Connection, sample_id: int) -> List[sqlite3.Row]:
    """Return readings for a given sample_id, compatible with both schemas."""
    mode = values_mode(db)
...
def q1(db: sqlite3.Connection, sql: str, params: Tuple[Any, ...] = ()) -> Optional[sqlite3.Row]:
    return one(db, sql, params)


# Helper: safe .get() for sqlite3.Row or dict
...
# ----------------------------
# Generic helpers used across routes/templates
# ----------------------------
...
def compute_target(tlow: Any, thigh: Any) -> Any:
    """Given (target_low, target_high), produce a single target value for display."""
    ...
    try:
        ...
        return thigh if thigh is not None else tlow
...
def insert_sample_reading(db: sqlite3.Connection, sample_id: int, pname: str, value: float, unit: str = "") -> None:
    ...
    if mode == "sample_values":
        ...
        cur.execute(
            "INSERT INTO parameters (sample_id, name, value, unit) VALUES (?, ?, ?, ?)",
            (sample_id, pname, value, unit or None),
        )
...
def parse_dt_any(v):
    """Parse SQLite/Excel datetime-ish values into a `datetime` (or None).

    Accepts:
      - datetime / date objects
      - ISO strings: 'YYYY-MM-DD', 'YYYY-MM-DD HH:MM', 'YYYY-MM-DDTHH:MM:SS', with optional 'Z'
      - SQLite common strings
      - epoch seconds or milliseconds (int/float or numeric strings)
    """
    if v is None:
        return None
    ...
    # Try ISO parser first (handles fractional seconds and timezone offsets)
    try:
...
# -------------------------------------------------
# DB init (lightweight, not full migrations)
# -------------------------------------------------

def values_mode(db: sqlite3.Connection) -> str:
    """Detect which schema stores parameter readings.

    Returns:
        'sample_values' if sample_values + parameter_defs exist
        'parameters' otherwise
    """
    try:
        cur = db.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('sample_values','parameter_defs')"
        )
        names = {r[0] for r in cur.fetchall()}
        if 'sample_values' in names and 'parameter_defs' in names:
...
def format_value(p: Any, v: Any) -> str:
    """Format a numeric value for display in templates."""
    if v is None:
        return ""
    try:
        fv = float(v)
        ...
        return str(v)
    # If parameter has a suggested precision, use it; else default 2 decimals but trim zeros.
    prec = None
    try:
        prec = getattr(p, "precision", None)
    except Exception:
        prec = None
    if prec is None:
        prec = 2
...
def init_db() -> None:
    ...
    cur.executescript(
        ...
        CREATE TABLE IF NOT EXISTS dose_plan_checks (
            ...
            checked_at TEXT,
            UNIQUE(tank_id, parameter, additive_id, planned_date),
            FOREIGN KEY (tank_id) REFERENCES tanks(id) ON DELETE CASCADE
        );
        """
    )

    # Safety for older DBs missing active columns
    ...
    # Per-parameter max daily change (used by dosing calculator to split doses over days)
    ensure_col(
        "parameter_defs",
        "max_daily_change",
        "ALTER TABLE parameter_defs ADD COLUMN max_daily_change REAL",
    )
    ...
    # Seed default parameters if none exist
    ...
    # init_db() should only ensure schema and seed data.
    # Any per-request computations (e.g., dose-plan totals) must happen inside request handlers,
    # not at import time.
    db.commit()
    db.close()
...
# -------------------------------------------------
# Routes: Dashboard / Tanks
# -------------------------------------------------
@app.get("/", response_class=HTMLResponse)
def dashboard(request: Request):
    db = get_db()
    tanks = q(db, "SELECT * FROM tanks ORDER BY name")
    tank_cards: List[Dict[str, Any]] = []
    for t in tanks:
        latest = one(
            db,
            "SELECT * FROM samples WHERE tank_id=? ORDER BY taken_at DESC LIMIT 1",
            (t["id"],),
        )
    ...
    db.close()
    # Dashboard has its own card/grid styling.
...
def add_reading_selector(request: Request):
    # Convenience route for older templates that link to /add
    db = get_db()
    tanks = q(db, "SELECT * FROM tanks ORDER BY name")
    db.close()
    html = """
    <html><head><title>Add Reading</title></head>
    <body style="font-family: sans-serif; max-width: 720px; margin: 40px auto;">
      <h2>Add Reading</h2>
      <form method="get" action="/tanks/0/add" onsubmit="event.preventDefault(); window.location = '/tanks/' + document.getElementById('tank').value + '/add';">
        <label>Select tank</label><br/>
        <select id="tank" name="tank" style="width:100%; padding:10px; margin:10px 0;" required>
          {options}
        </select>
    ...
    <script>
      // populate options via server-rendered HTML below
    </script>
    </body></html>
    """
...
def tank_new_form(request: Request):
    # Inline form (no extra template needed)
    html = """
    <html><head><title>Add Tank</title></head>
    <body style="font-family: sans-serif; max-width: 720px; margin: 40px auto;">
      <h2>Add Tank</h2>
      <form method="post" action="/tanks/new">
        <label>Tank name</label><br/>
        <input name="name" style="width:100%; padding:10px; margin:10px 0;" required />
        <label>Volume (L)</label><br/>
        <input name="volume_l" type="number" step="any" style="width:100%; padding:10px; margin:10px 0;" placeholder="e.g. 1500" />
...
DEFAULT_PARAMS: List[Tuple[str, str]] = [
    ("Alkalinity/KH", "dKH"),
    ("Calcium", "ppm"),
    ("Magnesium", "ppm"),
    ("Phosphate", "ppm"),
...
def tank_detail(request: Request, tank_id: int):
    ...
    # Normalize taken_at into datetime so templates can call strftime
    samples = []
    ...
    if samples:
        ...
        if mode == "sample_values":
            ...
            rows = q(
                ...
                LEFT JOIN parameter_defs pd ON pd.name = p.name
                WHERE p.sample_id IN ({placeholders})
                ORDER BY s.taken_at ASC
                """,
                tuple(sample_ids),
            )
        ...
        for r in rows:
            ...
            if value is not None:
                try:
                    ...
                    y = None
                if y is not None:
                    series_map.setdefault(name, []).append({"x": taken_at, "y": y})
    ...
    latest_by_param_id: Dict[str, Any] = {}
    if samples:
        latest_by_param_id = dict(sample_values.get(int(samples[0]["id"]), {}))

    targets_by_param: Dict[str, sqlite3.Row] = {t["parameter"]: t for t in targets if row_get(t, "parameter") is not None}
    status_by_param_id: Dict[str, str] = {}
    for pname in available_params:
        v = latest_by_param_id.get(pname)
        if v is None:
            status_by_param_id[pname] = "missing"
            continue
        t = targets_by_param.get(pname)
        if not t:
            continue
        al = row_get(t, "alert_low")
        ah = row_get(t, "alert_high")
        tl = row_get(t, "low")
        th = row_get(t, "high")
        try:
            fv = float(v)
        ...
        try:
            if al is not None and fv < float(al):
                status_by_param_id[pname] = "danger"
                continue
            if ah is not None and fv > float(ah):
                status_by_param_id[pname] = "danger"
                continue
            if tl is not None and fv < float(tl):
                status_by_param_id[pname] = "warn"
                continue
            if th is not None and fv > float(th):
                status_by_param_id[pname] = "warn"
                continue
    ...
    return templates.TemplateResponse(
        "tank_detail.html",
        {
            ...
            "latest_by_param_id": latest_by_param_id,
...
def tank_profile(request: Request, tank_id: int):
    ...
    if not profile:
        # Create a profile row, mirroring current volume if present.
        try:
            vol = tank["volume_l"] if "volume_l" in tank.keys() else None
        except Exception:
            vol = None
        db.execute(
            "INSERT OR IGNORE INTO tank_profiles (tank_id, volume_l, net_percent) VALUES (?, ?, 100)",
    ...
    # Provide a single dict that the template can use (volume_l/net_percent live in profile)
    tank_view = dict(tank)
    if profile:
        try:
...
async def tank_profile_save(request: Request, tank_id: int):
    ...
    # Update tank.volume_l (legacy) + profile row
    ...
    db.execute(
        ...
        "ON CONFLICT(tank_id) DO UPDATE SET volume_l=excluded.volume_l, net_percent=excluded.net_percent",
...
def add_sample_form(request: Request, tank_id: int):
    ...
    kits_by_param: Dict[str, List[sqlite3.Row]] = {}
    for k in kits:
...
# -------------------------------------------------
# Sample detail (view a single reading set)
# -------------------------------------------------
...
def sample_detail(request: Request, tank_id: int, sample_id: int):
    ...
    return templates.TemplateResponse(
        "sample_detail.html",
        {"request": request, "tank": tank, "sample": s, "readings": readings},
    )
...
async def sample_edit_save(request: Request, tank_id: int, sample_id: int):
    ...
    if taken_at:
        try:
            ...
            when_iso = None
    if not when_iso:
        when_iso = (parse_dt_any(sample["taken_at"]) or datetime.utcnow()).isoformat()
    ...
    if mode == "sample_values" and table_exists(db, "sample_values"):
        ...
        if table_exists(db, "parameters"):
            cur.execute("DELETE FROM parameters WHERE sample_id=?", (sample_id,))
...
def edit_targets(request: Request, tank_id: int):
        ...
        rows.append(
            {
                ...
                "key": slug_key(name),
                "target": compute_target(row_get(t, "target_low") if t else None, row_get(t, "target_high") if t else None),
                "target_low": (t["target_low"] if t and "target_low" in t.keys() else None),
                "target_high": (t["target_high"] if t and "target_high" in t.keys() else None),
                "alert_low": (t["alert_low"] if t and "alert_low" in t.keys() else None),
                "alert_high": (t["alert_high"] if t and "alert_high" in t.keys() else None),
    ...
    return templates.TemplateResponse(
        "edit_targets.html",
        {"request": request, "tank": tank, "tank_id": tank_id, "rows": rows},
    )
...
async def save_targets(request: Request, tank_id: int):
    ...
    # Ensure table + columns exist (safe for older DBs)
    cur.execute(
        '''
        CREATE TABLE IF NOT EXISTS targets (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    for p in params:
        ...
        # New single-target UI: if provided, store the same value in low/high for backward compatibility.
        ...
        # If all fields blank, or explicitly disabled, just disable (leave row if it exists)
        if (t_low is None and t_high is None and a_low is None and a_high is None) or enabled == 0:
            cur.execute(
                "UPDATE targets SET enabled=0 WHERE tank_id=? AND parameter=?",
                (tank_id, name),
            )
            continue
        ...
        cur.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(tank_id, parameter) DO UPDATE SET
...
# Backward-compatible aliases (some templates/old bookmarks use /tank/...)
@app.get("/tank/{tank_id}/targets", response_class=HTMLResponse, include_in_schema=False)
def edit_targets_alias(request: Request, tank_id: int):
    return edit_targets(request, tank_id)

@app.post("/tank/{tank_id}/targets", include_in_schema=False)
async def save_targets_alias(request: Request, tank_id: int):
    return await save_targets(request, tank_id)
...
def parameter_save(
    ...
    if param_id and str(param_id).strip().isdigit():
        ...
        new_name = name.strip()
        # If renaming, update related tables so we don't create duplicates.
        if old_name and new_name and old_name != new_name:
            ...
            # Update name-based tables
            for tbl, col in (("parameters", "name"), ("targets", "parameter"), ("additives", "parameter"), ("test_kits", "parameter")):
                if table_exists(db, tbl):
                    cur.execute(f"UPDATE {tbl} SET {col}=? WHERE {col}=?", (new_name, old_name))

            # If there's already a parameter_defs row with the new name, merge (keep the newest id).
            if existing_id is not None and existing_id != pid:
                keep_id = max(existing_id, pid)
                drop_id = min(existing_id, pid)

                # Ensure the kept row has the desired properties
                cur.execute(
                    "UPDATE parameter_defs SET name=?, unit=?, max_daily_change=?, sort_order=?, active=? WHERE id=?",
                    (*data, keep_id),
                )

                # Move sample_values to the kept id (if this schema is in use)
        ...
        # Create
        cur.execute(
            "INSERT INTO parameter_defs (name, unit, max_daily_change, sort_order, active) VALUES (?, ?, ?, ?, ?)",
            data,
        )
    db.commit()
    db.close()
...
def test_kit_new(request: Request):
    ...
    return templates.TemplateResponse(
        "test_kit_edit.html",
        {"request": request, "kit": None, "parameters": parameters},
    )
...
def test_kit_edit(request: Request, kit_id: int):
    ...
    return templates.TemplateResponse(
        "test_kit_edit.html",
        {"request": request, "kit": kit, "parameters": parameters},
    )
@app.post("/settings/test-kits/save")
def test_kit_save(
    kit_id: Optional[str] = Form(None),
    ...
    if kit_id and str(kit_id).strip().isdigit():
        cur.execute(
            "UPDATE test_kits SET parameter=?, name=?, unit=?, resolution=?, min_value=?, max_value=?, notes=?, active=? WHERE id=?",
            (*data, int(kit_id)),
        )
    else:
        cur.execute(
            "INSERT INTO test_kits (parameter, name, unit, resolution, min_value, max_value, notes, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            data,
        )
    db.commit()
    db.close()
...
# -------------------------------------------------
# Settings: Additives (template links to /settings/additives)
# -------------------------------------------------
...
@app.get("/settings/additives/", response_class=HTMLResponse, include_in_schema=False)
def additives_settings_redirect():
...
def additive_new(request: Request):
    ...
    return templates.TemplateResponse(
        "additive_edit.html",
        {"request": request, "additive": None, "parameters": parameters},
    )
...
def additive_edit(request: Request, additive_id: int):
    ...
    return templates.TemplateResponse(
        "additive_edit.html",
        {"request": request, "additive": additive, "parameters": parameters},
    )
@app.post("/additives/save")
def additive_save(
    additive_id: Optional[str] = Form(None),
    name: str = Form(...),
    parameter: Optional[str] = Form(None),
    parameter_id: Optional[str] = Form(None),
    strength: str = Form(...),
    unit: str = Form(...),
    max_daily: Optional[str] = Form(None),
    notes: Optional[str] = Form(None),
    active: Optional[str] = Form(None),
):
    db = get_db()
    ...
    if additive_id and str(additive_id).strip().isdigit():
        cur.execute(
            "UPDATE additives SET name=?, parameter=?, strength=?, unit=?, max_daily=?, notes=?, active=? WHERE id=?",
            (*data, int(additive_id)),
        )
    else:
        cur.execute(
            "INSERT INTO additives (name, parameter, strength, unit, max_daily, notes, active) VALUES (?, ?, ?, ?, ?, ?, ?)",
            data,
        )
    db.commit()
    db.close()
...
def presets(request: Request):
    ...
    return templates.TemplateResponse(
        "presets.html",
        {"request": request, "presets": rows, "created": False},
    )
...
def presets_create(
    ...
    return templates.TemplateResponse(
        "presets.html",
        {"request": request, "presets": rows, "created": True},
    )
...
def preset_detail(request: Request, preset_id: int):
    ...
    return templates.TemplateResponse(
        "preset_detail.html",
        {"request": request, "preset": preset, "items": items, "parameters": parameters},
    )


@app.post("/settings/presets/{preset_id}/items/create")
def preset_item_create(
    preset_id: int,
    ...
    db.execute(
        """
        INSERT INTO preset_items (preset_id, additive_name, parameter, strength, unit, max_daily, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (
            preset_id,
...
def preset_item_save(
    ...
    db.execute(
        """
        UPDATE preset_items
        SET additive_name=?, parameter=?, strength=?, unit=?, max_daily=?, notes=?
...
def dosing_log(request: Request, tank_id: int):
    ...
    logs = q(
        db,
        "SELECT dl.*, a.name AS additive_name "
        "FROM dose_logs dl "
        "LEFT JOIN additives a ON a.id = dl.additive_id "
        "WHERE dl.tank_id=? "
        "ORDER BY dl.logged_at DESC, dl.id DESC",
        (tank_id,),
    )
    ...
    return templates.TemplateResponse(
        "dosing_log.html",
        {"request": request, "tank": tank, "tank_id": tank_id, "additives": additives_rows, "logs": norm_logs},
    )
...
async def dosing_log_add(request: Request, tank_id: int):
    ...
    cur.execute(
        "INSERT INTO dose_logs (tank_id, additive_id, amount_ml, reason, logged_at) VALUES (?, ?, ?, ?, ?)",
        (tank_id, aid, float(amount_ml), reason, when_iso),
    )
    db.commit()
    db.close()
    return redirect(f"/tanks/{tank_id}/dosing-log")
...
# -------------------------------------------------
# Admin: Merge duplicate parameters (created after renames)
# -------------------------------------------------
...
def merge_parameters_page(request: Request):
    ...
    for key, items in groups.items():
        if key and len(items) > 1:
            ...
            keep = items_sorted[-1]  # newest definition
            drop = items_sorted[:-1]
            dups.append({"key": key, "keep": keep, "drop": drop})

    db.close()
    return templates.TemplateResponse(
        "merge_parameters.html",
        {"request": request, "dups": dups},
    )
...
def merge_parameters_run():
    ...
    for key, items in groups.items():
        ...
        for drop in items_sorted[:-1]:
            ...
            for tbl, col in (("parameters", "name"), ("targets", "parameter"), ("additives", "parameter"), ("test_kits", "parameter")):
                if table_exists(db, tbl):
                    cur.execute(f"UPDATE {tbl} SET {col}=? WHERE {col}=?", (keep_name, drop_name))

            # id-based readings (sample_values schema)
            if table_exists(db, "sample_values"):
                cur.execute("UPDATE sample_values SET parameter_id=? WHERE parameter_id=?", (keep_id, drop_id))
...
def calculators(request: Request):
    ...
    return templates.TemplateResponse(
        "calculators.html",
        {"request": request, "tanks": tanks, "additives": additives_rows, "profiles": profile_by_tank},
    )


@app.post("/tools/calculators", response_class=HTMLResponse)
def calculators_post(
    request: Request,
    tank_id: int = Form(...),
    additive_id: int = Form(...),
    desired_change: float = Form(...),
):
    db = get_db()
    tank = one(db, "SELECT * FROM tanks WHERE id=?", (tank_id,))
    ...
    net_percent = (
        (tank_profile["net_percent"] if tank_profile and tank_profile["net_percent"] is not None else 100)
        if tank_profile
        else 100
    )
    ...
    daily_change = None
    unit = ""
    if volume_l is None or strength in (None, 0):
        error = "Tank volume or additive strength is missing/invalid."
    else:
        volume = float(volume_l) * (float(net_percent) / 100.0)
        dose_ml = (float(desired_change) / float(strength)) * (volume / 100.0)
        # Daily limit (max change per day) comes from Settings → Parameters
        # NOTE: older versions stored an additive "max_daily" in mL; we intentionally ignore that so it doesn't break splitting.
        # Look up daily max change for this additive's parameter (stored on parameter_defs)
        pname = (additive['parameter'] or '').strip()
        pdef = one(db, "SELECT * FROM parameter_defs WHERE name=?", (pname,))
        if not pdef and pname:
            # Fallback: match on substring (e.g. 'Alkalinity/KH' should match 'Alkalinity')
    ...
    return templates.TemplateResponse(
        "calculators.html",
        {
            "request": request,
            "result": {
                "dose_ml": None if dose_ml is None else round(dose_ml, 2),
                "days": days,
                "daily_ml": None if daily_ml is None else round(daily_ml, 2),
...
# -------------------------------------------------
# Tools: Dose plan (targets vs latest)
# -------------------------------------------------
...
def dose_plan(request: Request):
    """Show per-tank dosing plan to reach target values based on latest readings."""
    db = get_db()
    today = date.today()
    # Preload checkbox state for upcoming doses (next 60 days)
    try:
        chk_rows = q(
            db,
    ...
    # parameter defs map for units and max daily change (in parameter unit per day)
    ...
    for t in tanks:
        ...
        # effective volume = profile volume * net%
        ...
        if prof:
            ...
            try:
                net_pct = float(prof["net_percent"] if prof["net_percent"] is not None else 100.0)
            except Exception:
                net_pct = 100.0
        if vol_l is None:
            vol_l = t["volume_l"] if ("volume_l" in t.keys() and t["volume_l"] is not None) else None

        eff_vol_l = None
        try:
            if vol_l is not None:
                eff_vol_l = float(vol_l) * (net_pct / 100.0)
        except Exception:
            eff_vol_l = None
        ...
        if latest:
            ...
            try:
                for r in get_sample_readings(db, int(latest["id"])):
                    latest_readings[r["name"]] = r["value"]
            except Exception:
                latest_readings = {}
        ...
        for tr in targets:
            ...
            # Determine target value: mid of low/high if both, else whichever provided
            ...
            try:
                if tl is not None and th is not None:
                    target_val = (float(tl) + float(th)) / 2.0
                elif th is not None:
                    target_val = float(th)
                elif tl is not None:
                    target_val = float(tl)
            ...
            unit = ""
            if pname in pdef_map and pdef_map[pname]["unit"]:
                unit = pdef_map[pname]["unit"]
            elif "unit" in tr.keys() and tr["unit"]:
                unit = tr["unit"]

            # max daily change in unit/day (from parameter defs)
            max_change = None
            if pname in pdef_map:
                try:
                    max_change = pdef_map[pname]["max_daily_change"]
                except Exception:
                    max_change = None
            try:
                max_change_f = float(max_change) if max_change is not None else None
            except Exception:
                max_change_f = None

            days = 1
            if max_change_f and max_change_f > 0 and delta > max_change_f:
                days = int(math.ceil(delta / max_change_f))

            per_day_change = delta / float(days)

            # Pick matching additives
                ...
                tank_rows.append(
                    {
                        ...
                        "additives": [],
                        "note": "No additive linked to this parameter.",
                    }
                )
                continue
            ...
            for a in adds:
                strength = a["strength"]
                if strength in (None, 0) or eff_vol_l is None:
                    total_ml = None
                else:
                    try:
                        total_ml = (float(delta) / float(strength)) * (float(eff_vol_l) / 100.0)
                    except Exception:
                        total_ml = None
        ...
        # Total planned dose for the whole tank across all planned additives
        plan_total_ml = 0.0
        for _r in tank_rows:
            for _a in (_r.get("additives") or []):
                try:
                    plan_total_ml += float(_a.get("total_ml") or 0)
                except Exception:
                    pass
        plans.append(
            {
...
# -------------------------------------------------
# Import from bundled "Tank Parameters Sheet.xlsx"
# -------------------------------------------------

def import_from_tank_parameters_sheet(db: sqlite3.Connection, xlsx_path: str) -> Dict[str, Any]:
    """Import readings and tank volumes from the provided Excel sheet.
    Expected format:
      - One sheet per tank, with headers in row 1:
          Column 1 | <Parameter 1> | <Parameter 2> | ... | Tank Volume | <volume value>
      - Data rows from row 2: date/time in column 1, numeric values under parameter columns.
    """
    try:
        import openpyxl  # type: ignore
    except Exception as e:  # pragma: no cover
        raise RuntimeError("openpyxl is required for Excel import. Add it to requirements and rebuild the container.") from e
    ...
    def get_or_create_tank(name: str, volume_l: float | None) -> int:
        nonlocal imported
        r = one(db, "SELECT id, volume_l FROM tanks WHERE name=?", (name,))
        if not r:
            cur.execute("INSERT INTO tanks (name, volume_l) VALUES (?, ?)", (name, volume_l))
            tid = cur.lastrowid
            imported["tanks_created"] += 1
        else:
            tid = r["id"]
            # Update volume if provided and different/empty
            if volume_l is not None:
                try:
                    existing = r["volume_l"]
                except Exception:
                    existing = None
                if existing is None or float(existing or 0) != float(volume_l):
                    cur.execute("UPDATE tanks SET volume_l=? WHERE id=?", (volume_l, tid))
                    imported["tanks_updated"] += 1
        # Ensure profile row exists & mirror volume
        if table_exists(db, "tank_profiles"):
            pr = one(db, "SELECT tank_id FROM tank_profiles WHERE tank_id=?", (tid,))
            if not pr:
                cur.execute("INSERT INTO tank_profiles (tank_id, volume_l, net_percent) VALUES (?, ?, 100)", (tid, volume_l))
            else:
                if volume_l is not None:
                    cur.execute("UPDATE tank_profiles SET volume_l=? WHERE tank_id=?", (volume_l, tid))
        return tid
    ...
    def parse_dt(v) -> str | None:
        ...
        if isinstance(v, (int, float)):
            try:
                return openpyxl.utils.datetime.from_excel(v).isoformat()
            except Exception:
                return None
        return None
    ...
    for sheet_name in wb.sheetnames:
        ...
        # Find "Tank Volume" marker
        vol_col = None
        for c, h in headers:
            if h.lower() == "tank volume":
                vol_col = c
                break

        volume_l = None
        if vol_col is not None:
            vol_val = ws.cell(1, vol_col + 2).value if ws.max_column >= (vol_col + 2) else ws.cell(1, vol_col + 1).value
            try:
                volume_l = float(vol_val) if vol_val is not None else None
            except Exception:
                volume_l = None

        # Parameter columns are from col 2 until tank volume marker (exclusive)
        param_cols: list[tuple[int, str]] = []
        for c, h in headers:
            if c == 1:
                continue
            if vol_col is not None and c >= vol_col:
                break
            # Ignore generic placeholders
            if h.lower() in {"column 1", "date", "datetime"}:
                continue
            param_cols.append((c, h))

        if not param_cols:
            continue

        tank_id = get_or_create_tank(sheet_name, volume_l)
        imported["sheets_processed"] += 1

        # Insert readings
        for r in range(2, ws.max_row + 1):
            when_iso = parse_dt(ws.cell(r, 1).value)
            if not when_iso:
                continue

            # Create sample row
            cur.execute("INSERT INTO samples (tank_id, taken_at, notes) VALUES (?, ?, ?)", (tank_id, when_iso, None))
            sample_id = cur.lastrowid
            imported["samples_created"] += 1
            ...
            for c, pname in param_cols:
                ...
                unit_guess = ""
                lname = pname.lower()
                if "alk" in lname or "d kh" in lname or "alkalinity" in lname:
                    unit_guess = "dKH"
                elif "calcium" in lname:
                    unit_guess = "ppm"
                elif "magnesium" in lname:
                    unit_guess = "ppm"
                elif "nitrate" in lname:
                    unit_guess = "ppm"
                elif "phosphate" in lname:
                    unit_guess = "ppm"
                ...
                insert_sample_reading(db, sample_id, pname, float(fv), unit_guess)
                imported["values_created"] += 1
...
async def dose_plan_check(request: Request):
    """Toggle a checkbox for a planned dose day."""
    form = await request.form()

    # Allow the frontend to send a single composite key (older template versions)
    # key format: <tank_id>|<parameter>|<additive_id>|<planned_date>
    key = (form.get("key") or "").strip()
    if key and (not form.get("tank_id")):
        parts = key.split("|", 3)
        if len(parts) == 4:
            form = {**form, "tank_id": parts[0], "parameter": parts[1], "additive_id": parts[2], "planned_date": parts[3]}
    try:
        tank_id = int(form.get("tank_id") or 0)
        additive_id = int(form.get("additive_id") or 0)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid tank/additive id")

    parameter = (form.get("parameter") or "").strip()
    planned_date = (form.get("planned_date") or "").strip()  # YYYY-MM-DD
    checked = 1 if str(form.get("checked") or "").lower() in ("1", "true", "on", "yes") else 0

    # optional amount (ml) for logging when checked
    try:
    ...
    db.execute(
        """
        INSERT INTO dose_plan_checks (tank_id, parameter, additive_id, planned_date, checked, checked_at)
        VALUES (?, ?, ?, ?, ?, ?)
        ON CONFLICT(tank_id, parameter, additive_id, planned_date) DO UPDATE SET
    ...
    # If checked, also record to dose_logs (and remove if unchecked)
    try:
        ...
        if checked and amount_ml > 0:
            existing = one(
                db,
                "SELECT id FROM dose_logs WHERE tank_id=? AND additive_id=? AND logged_at=? AND reason=? LIMIT 1",
                (tank_id, additive_id, logged_at, reason),
            )
            if not existing:
                db.execute(
                    "INSERT INTO dose_logs (tank_id, additive_id, amount_ml, reason, logged_at) VALUES (?, ?, ?, ?, ?)",
                    (tank_id, additive_id, amount_ml, reason, logged_at),
                )
        elif not checked:
            db.execute(
                "DELETE FROM dose_logs WHERE tank_id=? AND additive_id=? AND logged_at=? AND reason=?",
                (tank_id, additive_id, logged_at, reason),
            )
    except Exception:
        # Never fail the checkbox toggle due to log issues
        pass
    db.commit()
    db.close()
...
def import_excel_page(request: Request):
    # Simple page with a button to run import
    return templates.TemplateResponse(
        "simple_message.html",
        {
            "request": request,
            "title": "Import Tank Parameters Sheet",
            "message": "This will import tank volumes and historical readings from the bundled Tank Parameters Sheet.xlsx.",
...
async def import_excel_run(request: Request):
    ...
    # Try to locate the Excel file in common locations. You can override with REEF_EXCEL_PATH.
    candidates = []
    ...
    candidates += [
        os.path.join(here, "Tank Parameters Sheet.xlsx"),              # /app/Tank Parameters Sheet.xlsx
        os.path.join(here, "..", "Tank Parameters Sheet.xlsx"),        # repo root one up
        os.path.join(here, "..", "..", "Tank Parameters Sheet.xlsx"),  # repo root two up (zip layout)
        os.path.join(os.getcwd(), "Tank Parameters Sheet.xlsx"),
        os.path.join(os.getcwd(), "reef-params", "Tank Parameters Sheet.xlsx"),
    ]
    ...
    if not xlsx_path:
        ...
        return templates.TemplateResponse(
            "simple_message.html",
            {
                "request": request,
                "title": "Import failed",
                "message": "Could not find 'Tank Parameters Sheet.xlsx'. Place it in /app (same folder as main.py), "
                           "or set REEF_EXCEL_PATH to the full path inside the container.",
                "actions": [{"label": "Back", "method": "get", "href": "/"}],
            },
        )
    ...
    try:
        ...
        # Render a friendly error rather than 500-ing with no context.
        db.close()
        return templates.TemplateResponse(
            "simple_message.html",
            {
                "request": request,
                "title": "Import failed",
                "message": f"Error while importing from Excel: {type(e).__name__}: {e}",
                "actions": [{"label": "Back", "method": "get", "href": "/admin/import-excel"}],
            },
        )
    finally:
        try:
    ...
    return templates.TemplateResponse(
        "simple_message.html",
        {
            ...
            "message": f"Sheets processed: {stats.get('sheets_processed', 0)} | Tanks created: {stats.get('tanks_created', 0)} | "
                       f"Samples: {stats.get('samples_created', 0)} | Values: {stats.get('values_created', 0)}",
            "actions": [{"label": "Go to dashboard", "method": "get", "href": "/"}],
        },
    )